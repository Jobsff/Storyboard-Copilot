...
5. [Agnes AI](https://agnes-ai.com)：当前接入图片、视频与 AI 助手能力，适合测试免费/低成本多模态工作流
...
说明：
- `gpt-image-2` 不稳定支持真实 Alpha 透明背景。序列帧资产建议走纯色绿幕背景，再通过「切割动画」的自动/强制抠图生成透明帧。

---

# Agnes AI

Agnes AI 使用独立供应商配置，当前支持：
- 图片生成/编辑：`agnes-image-2.1-flash`
- 视频生成：`agnes-video-v2.0`
- AI 助手文本能力：`agnes-2.0-flash`

配置步骤：
1. 在 Agnes AI 官网创建并复制 API Key
2. 在「设置 → 服务商」中找到 Agnes AI
3. 将密钥填写到 Agnes AI 的密钥输入框

说明：
- Agnes 图片模型支持文生图与图生图，实际透明背景效果取决于模型返回结果。
- Agnes 视频模型可用于文生视频；如果节点连入参考图，也会按模型能力走图生视频流程。
- AI 助手服务商选择 Agnes AI 时，默认模型建议填写 `agnes-2.0-flash`。
...
2. 从下拉框选择服务商（666API / 巨游API / Agnes AI / Ollama）
