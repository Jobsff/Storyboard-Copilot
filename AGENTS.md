...
6. 默认工作模式（2026-09-14 用户确认，优先级高于个人习惯）

- **主对话零执行**：主对话只承载需求讨论、方案决策与验收；一切改代码/构建/打包类执行活由主对话派子智能体后台完成（`run_in_background`，派活前一行播报「谁+干什么」，完成后一行汇报，不打断主对话）。
  验收口径：任何代码变更都有对应后台任务小票；派活后主对话立即可继续交互。
- **批次收尾即打包**：每个功能批次验证收尾（`npx tsc --noEmit` / `npm test` / `npm run build` 含 check-bundle / 无头 Chrome 冒烟）后，默认立即本地打包 macOS dmg，**挂载弹出安装窗口（open dmg）后即停**——关闭旧版应用与覆盖安装到 /Applications 由用户手动完成，智能体不做、后续不用管。
  验收口径：批次收尾必有新版本号 dmg 路径 + 安装窗口已弹出。
- **Windows 与 GitHub 冻结**：未经用户明确指令，不做 git commit / push / tag，不触发 Windows CI，不动 GitHub Release。用户全部测试完成并明示推送（见上方第 5 条口令）后才走发版链路。
...
- AI 序列帧默认使用纯洋红 `#FF00FF` 抠图背景；切割动画后处理需兼容洋红/绿/蓝 chroma key、白底清理、已有 Alpha 但残留 chroma 的帧。
...
- 序列帧游戏导出优先产出 `frames/ + *_sheet.png + frames.json`；Spine 导出必须产出可被现有导入器重新导入的 frame-by-frame attachment `.json + .atlas + .png` 文件包，并在导出后能创建/验证预览节点。
...
- Spine 资产字段通过 `assetPool + __asset_ref__` 做去重编码；新增资产字段需同步编码/解码映射。
- 禁止在项目保存、删除项目或节点删除链路中自动删除本地图片/Spine 资产物理文件；清理磁盘素材必须是用户明确触发的手动操作。
- 变更 SQLite 表结构时：
